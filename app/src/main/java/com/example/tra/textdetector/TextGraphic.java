...
package com.example.tra.textdetector;
...
public class TextGraphic extends GraphicOverlay.Graphic {
    ...
    public static boolean checkFavoriteItem(String wallpaperID, Context context) {
        boolean check = false;
        List<String> favorites = getArrayList("dataList", context);
        if (favorites != null) {
            for (String product : favorites) {
                if (product.equals(wallpaperID)) {
                    check = true;
                    break;
                }
            }
    ...
    public void draw(Canvas canvas) {
        ...
        for (TextBlock textBlock : text.getTextBlocks()) {
            ...
            String[] splitText = textBlock.getText().split(" ");
            ...
            for (int i = 0; i < splitText.length; i++) {
                ...
                for (Line line : textBlock.getLines()) {
                    ...
                    for (Element element : line.getElements()) {
                        ...
                        if (checkFavoriteItem(element.getText().toLowerCase(), getApplicationContext()) ||
                                checkFavoriteItem(element.getText(), getApplicationContext())
                                || checkFavoriteItem(element.getText().toUpperCase(), getApplicationContext())) {
                            String text = showLanguageTag
                                    ? String.format(
                                    TEXT_WITH_LANGUAGE_TAG_FORMAT, line.getRecognizedLanguage(), element.getText())
                                    : element.getText();
                            drawText(text, new RectF(element.getBoundingBox()), TEXT_SIZE + 2 * STROKE_WIDTH, canvas);
                        }
